...
public class TableServiceFactoryTest {
  ...
  private @Mock CircusTrainBigQueryConfiguration configuration = new CircusTrainBigQueryConfiguration();
  ...
  public void newInstanceCachesService() {
    ...
    this.tableServiceFactory = new TableServiceFactory(configuration, bigQueryMetastore, service, map,
        partitionQueryFactory);
    Table table = new Table();
    when(partitionQueryFactory.get(table)).thenReturn("foo > 5");
